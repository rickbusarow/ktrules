...
/** @since 1.0.7 */
internal fun ASTNode.prevSiblings(): Sequence<ASTNode> = generateSequence(prevSibling()) {
  it.prevSibling()
}
...
/** @since 1.0.7 */
internal fun ASTNode.nextSiblings(): Sequence<ASTNode> = generateSequence(nextSibling()) {
  it.nextSibling()
}
 ...
 * @since 1.0.4
 */
internal fun ASTNode.childrenDepthFirst(predicate: (ASTNode) -> Boolean): Sequence<ASTNode> =
 ...
 * @since 1.0.4
 */
internal fun ASTNode.childrenBreadthFirst(predicate: (ASTNode) -> Boolean): Sequence<ASTNode> =
 ...
 * @since 1.1.0
 */
internal fun ASTNode.parents(): Sequence<ASTNode> = generateSequence(
  treeParent
) { node -> node.treeParent }

/** @since 1.1.1 */
internal inline fun <T : ASTNode> T.removeFirstChildrenWhile(
  shouldRemove: (ASTNode) -> Boolean
): T {
  return apply {
    children()
      .toList()
...
/** @since 1.1.1 */
internal inline fun <T : ASTNode> T.removeLastChildrenWhile(shouldRemove: (ASTNode) -> Boolean): T {
  return apply {
    children()
      .toList()
...
/** @since 1.0.7 */
internal inline fun <T : ASTNode> T.removeAllChildren(
...
/** @since 1.0.7 */
internal fun <T : ASTNode> T.removeAllChildrenRecursive(shouldRemove: (ASTNode) -> Boolean): T {
  return apply {
