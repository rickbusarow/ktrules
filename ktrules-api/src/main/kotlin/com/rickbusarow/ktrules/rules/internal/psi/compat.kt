 ...
 * @since 1.1.0
 */
internal fun ASTNode.prevLeafAny(): ASTNode? {
 ...
 * @since 1.1.0
 */
internal inline fun ASTNode.prevSibling(predicate: (ASTNode) -> Boolean = { true }): ASTNode? {
  var n = this.treePrev
  while (n != null) {
    if (predicate(n)) {
      return n
 ...
 * @since 1.1.0
 */
internal inline fun ASTNode.nextSibling(predicate: (ASTNode) -> Boolean = { true }): ASTNode? {
  var n = this.treeNext
  while (n != null) {
    if (predicate(n)) {
      return n
 ...
 * @since 1.1.0
 */
internal fun ASTNode.nextLeafAny(): ASTNode? {
  var n = this
 ...
 * @since 1.1.0
 */
internal fun ASTNode.nextLeafStrict(): ASTNode? {
 ...
 * @since 1.1.0
 */
internal fun ASTNode.isCodeLeaf(): Boolean = isLeaf() && !isWhiteSpace() && !isPartOfComment()

/**
 * @return true if the node is part of a KDoc comment, block comment, or end-of-line comment
 * @since 1.1.0
 */
internal fun ASTNode.isPartOfComment(): Boolean = parent(strict = false) {
 ...
 * @since 1.1.0
 */
internal fun ASTNode.upsertWhitespaceBeforeMe(text: String) {
 ...
 * @since 1.1.0
 */
internal fun ASTNode.upsertWhitespaceAfterMe(text: String) {
  if (this is LeafElement) {
    if (this.elementType == ElementType.WHITE_SPACE) {
      return replaceWhitespaceWith(text)
    }
 ...
 */
internal fun ASTNode.replaceWhitespaceWith(text: String) {
 ...
 * @since 1.1.0
 */
internal fun ASTNode.isPartOfString(): Boolean = parent(ElementType.STRING_TEMPLATE, strict = false) != null

/**
 * @return true if the [ASTNode] is white space, false otherwise.
 * @since 1.1.0
 */
internal fun ASTNode?.isWhiteSpace(): Boolean = this != null && elementType == ElementType.WHITE_SPACE

/**
 * @return true if the [ASTNode] is white space with a newline, false otherwise.
 * @since 1.1.0
 */
internal fun ASTNode?.isWhiteSpaceWithNewline(): Boolean =
 ...
 * @since 1.1.0
 */
internal fun ASTNode?.isRegularStringWithNewline(): Boolean =
 ...
 * @since 1.1.0
 */
internal fun ASTNode?.isWhiteSpaceWithoutNewline(): Boolean =
  this != null && elementType == ElementType.WHITE_SPACE && !textContains('\n')
 ...
 * @since 1.1.0
 */
internal fun ASTNode.isRoot(): Boolean = elementType == ElementType.FILE

/**
 * @return true if the [ASTNode] is a leaf (has no children), false otherwise.
 * @since 1.1.0
 */
internal fun ASTNode.isLeaf(): Boolean = firstChildNode == null
